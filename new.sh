...
function portscan
{
  tput setaf 6; echo "Starting port scan of $listfile port 80"; tput sgr0;
  if nc -zw1 $listfile  80; then
    tput setaf 2; echo "Port scan good, $listfile port 80 available"; tput sgr0;
  else
    echo "Port scan of $listfile port 80 failed."
  fi
}
...
function pingnet
{
  ...
  tput setaf 6; echo "Pinging $listfile to check for internet connection." && echo; tput sgr0;
  ping $listfile -c 4

  if [ $? -eq 0 ]
    then
      tput setaf 2; echo && echo "$listfile pingable. Internet connection is most probably available."&& echo ; tput sgr0;
...
function httpreq
{
  ...
  case "$(curl -s --max-time 2 -I $listfile | sed 's/^[^ ]*  *\([0-9]\).*/\1/; 1q')" in
